...
import { VeoExtendVideoNodeData } from "@/types";

type VeoExtendNodeType = Node<VeoExtendVideoNodeData, "veoExtendVideo">;

export function VeoExtendVideoNode({ id, data, selected }: NodeProps<VeoExtendNodeType>) {
  const nodeData = data;
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Left} id="taskId" data-handletype="text" style={{ top: "30%" }} />
      ...
      <div className="absolute text-[10px] font-medium whitespace-nowrap pointer-events-none" style={{ right: `calc(100% + 8px)`, top: "calc(30% - 18px)", color: "var(--handle-color-text)" }}>
        TaskId
      </div>
        ...
        {nodeData.outputVideo ? (
          ...
          <div className="w-full flex-1 min-h-[96px] border border-dashed border-neutral-600 rounded flex items-center justify-center">
            {nodeData.status === "loading" ? (
              <span className="text-[10px] text-neutral-400">Extending...</span>
            ) : nodeData.status === "error" ? (
              <span className="text-[10px] text-red-400 text-center px-2">{nodeData.error || "Failed"}</span>
            ) : (
              <span className="text-[10px] text-neutral-500">Run to extend</span>
            )}
          </div>
        )}
      </div>
